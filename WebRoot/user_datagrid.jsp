...
<script type="text/javascript">
  ...
  function userUpdate(arg){
      ...
      location.href = "UserRoleFindByUserIDServlet?userid="+userid+"";
  }
</script>
...
<body>
	...
	<table class="datagridtable" align="center" style="text-align: center;"
		...
		<%
			int temp = 0;
			for (UserVO user : (List<UserVO>) pageVO.getLists()) {
				temp++;
				if (temp % 2 == 0) {
		%>
		<tr style="background-color: #00bcd4;">
			...
			<td>
				<button userid="<%= user.getId() %>"
					onclick="userUpdate(this)">修改</button></td>
		</tr>
		<%
			} else {
		%>
		<tr style="background-color: #00bcd4;">
			...
			<td><%=user.getRolename()%></td>
			<td>
				<button userid="<%= user.getId()%>"  onclick="userUpdate(this)">修改</button></td>
		</tr>
		<%
			}
			}
