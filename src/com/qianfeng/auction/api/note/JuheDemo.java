...
public class JuheDemo {
	...
	public static void getRequest1() {
		...
		params.put("word", "");// �?���?��的短信内容，�?��UTF8 URLENCODE
	...
	// 2.发�?短信
	public static int getRequest2(String phoneNumber) {
		String result = null;
		String url = "http://v.juhe.cn/sms/send";// 请求接口地址
		Map params = new HashMap();// 请求参数
		params.put("mobile", phoneNumber);// 接收短信的手机号�?
		params.put("tpl_id", "108275");// 短信模板ID，请参�?个人中心短信模板设置
		int validateCode = (int) (Math.random() * 9000 + 1000);
		params.put("tpl_value", "#code#=" + validateCode + "");// 变量名和变量值对。如果你的变量名或�?变量值中带有#&=中的任意�?��特殊符号，请先分别进行urlencode编码后再传�?�?a
		// href="http://www.juhe.cn/news/index/id/50"
		// target="_blank">详细说明></a>
		params.put("key", APPKEY);// 应用APPKEY(应用详细页查�?
		params.put("dtype", "");// 返回数据的格�?xml或json，默认json
		try {
			result = net(url, params, "GET");
			JSONObject object = JSONObject.fromObject(result);
			if (object.getInt("error_code") == 0) {
